...
def main( folder_path, keypoint_model_weights,classifier):
    ...
    while cap.isOpened():
        ...
        """
        #2 inputs inference code 
        X_test_x = [kp for person in normalized_keypoints.values() for kp_list in person.values() for kp in kp_list]
        ...
        #     x_coords = [kp for kp_list in person_keypoints.values() for kp in kp_list]
        #     y_coords = [kp for kp_list in person_keypoints.values() for kp in kp_list]
        ...
        for i, (pred1,pred2) in enumerate(paired_predictions):
            # if prediction[0] > 0.5  # Assuming a threshold for binary classification
                # print("person num:", person_number[i])
                # person_label = f"Person{person_number[i]}: Hand Raised"
            if pred1 > 0.5 and pred2 > 0.5:
                cv2.putText(frame, "Hand Raised", (50 + i * 30, 200),
